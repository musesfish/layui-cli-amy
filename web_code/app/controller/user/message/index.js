; layui.define( function(e){
    ...
    layui.use(['jquery', 'form', 'api', 'ajax', 'renders', 'element', 'table',  'laytpl', 'laydate', 'selPersonnel'], function () {
        ...
        function create() {
            ...
            '<div class="layui-btn w100" lay-event="del" >批量删除</div>' + 
            '<div class="layui-btn w100" lay-event="addmess" >添加</div>'
            '</div>' +
            ...
            var toolrowTemp = '<div><div class="btn-groups">' +
                '<span lay-event="examine" >查看</span>' +
                '</div></div>';
            ...
            var col = [[
                ...
                { field: 'noticeTitle', title: '标题', templet: '<div><span class="cursor-pointer" lay-event="examine">{{d.noticeTitle}}</span></div>' },
                { field: 'noticeTypeName', title: '通知类型', },
                { field: 'userName', title: '创建人', },
                { field: 'createDate', title: '创建日期', templet: function (d) { return api.splitTimeDate(d.createDate) } },
                { field: 'state', title: '状态',templet: '<div>{{# if(d.state==0){ }}<span class="cursor-pointer" lay-event="examine">已读</span>{{# } else { }}<span class="cursor-pointer fc-red" lay-event="examine">未读</span>{{# } }}</div>' },
                { title: '操作', templet: toolrowTemp, width: 160, fixed: 'right' }
            ]];
            var col1 = [[
                { checkbox: true, fixed: 'left' },
                { field: 'id', title: '编号', width: 120 },
                { field: 'noticeTitle', title: '标题', templet: trTitleTemp },
                { field: 'noticeTypeName', title: '通知类型', templet: trTypeTemp },
                { field: 'createDate', title: '创建日期', templet: function (d) { return api.splitTimeDate(d.createDate) } },
                { title: '操作', templet: toolrowTemp1, width: 160, fixed: 'right' }
            ]];
            ...
            table.on('tool(personnelcenter-message-table)', function (obj) {
                switch (obj.event) {
                    ...
                    break;
                    case 'delete':
                        layer.confirm('确认删除这条通知吗？', { icon: 0, skin: 'layer-theme-confirm', }, function () {
                            ajax({
            ...
            table.on('toolbar(personnelcenter-message-table)', function (obj) {
                ...
                switch (obj.event) {
                    case 'del':  // 批量删除
                        ...
                        var data = arr;
                        layer.confirm('确认删除所有选中的通知吗？', { icon: 0, skin: 'layer-theme-confirm', }, function () {
                            ajax({
                                type: 'JSON',
            ...
            function addPersonnel() {
                ...
                var query = [
                    // 'r=' + (authControl[control.indexOf('p添加人员')].roleId) || '',
                    'all=1',
                    "m=1"
                ];
