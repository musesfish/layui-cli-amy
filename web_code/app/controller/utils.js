 ...
 * author: hoovoi
 * 工具类：数据类型校验转化
 * @Function verifyData(<源数据对象>,<数据类型对象>,<options配置>) - 支持多层级的Json对象数据校验转换
 * @Function verifyDataTypes(<源数据>,<数据类型>) - 单一数据的校验转换
 * 
 * 数据类型
 * 普通类型： array、string、number、boolean、object
 * 扩展类型： date(yyyy-mm-dd)、time(hh:mm:ss)、month(yyyy-mm)、货币 money(0.00)
 * 非法格式则输出对应的空类型数据
 * 
 * options配置
 * hasEmpty: false - 是否保留属性值为空的键值
 * filterEmpty: Array - 保留的键值
*/
;layui.define(function (exports) {
    const MOD_NAME = "utils";   
    let options = {
        hasEmpty: false, // 是否保留属性空值属性
        filterEmpty: [] // 保留空值的属性键值
    };
     ...
     * @param {*} d - 校验数据
     * @param {*} dt - 校验数据的格式
     * @param {json} opt - { hasEmpty: false, filterEmpty: [key1, key2] }
     */
    function verifyData(d, dt, opt){
        Object.assign(options, opt||{});
        return verifyDataInit(d, dt);
    };

    function verifyDataInit(d, dt){
        if (typeof d === 'object' && !Array.isArray(d)) {
            for (var i in d) {
                if (typeof d[i] === 'object' && !Array.isArray(d[i]) && d[i] != null) {
                    verifyDataInit(d[i], dt[i], options)
                } else if (!options.hasEmpty && !d[i] && options.filterEmpty.indexOf(i) === -1) {
                    delete d[i]
                } else {
                    d[i] = verifyDataTypes(d[i], dt[i], options)
                }
            }
        } else {
            d = verifyDataTypes(d, dt)
        }
        return d;
    }

    // 校验数据类型
    function verifyDataTypes($d, $dt) {
        var dt = $dt || 'string'; // 默认为string
        var d = $d || ''; // 过滤undefined和null
        switch (dt) {
            case 'object': d = !d?{}:typeof d === 'object'?d:{}; break;
            case 'string': d = d + ''; break;
            case 'number': d = d - 0; break;
            case 'money': d = (d - 0).toFixed(2); break;
            case 'boolean': d = d?!1:!0; break;
            case 'date': d = convertTime(d, 'date'); break;
            case 'month': d = convertTime(d, 'month'); break;
            case 'time': d = convertTime(d, 'time'); break;
            case 'array': d = Array.isArray(d)? d: [d]; break;
        }
        return d;
    };
     ...
     * 时间格式转换
     */
    function convertTime(d, format) {
        if (!d) return '';
        var t = new Date(d);
        if (isNaN(t.getFullYear())) return '';
        switch (format) {
            case 'YMD':
                t =  `${t.getFullYear()}-${layui.api.markZero(t.getMonth() + 1, 2)}-${layui.api.markZero(t.getDate(), 2)}`;
                break;
            case 'date':
                t = `${t.getFullYear()}-${layui.api.markZero(t.getMonth() + 1, 2)}-${layui.api.markZero(t.getDate(), 2)} ${layui.api.markZero(t.getHours(), 2)}:${layui.api.markZero(t.getMinutes(), 2)}:${layui.api.markZero(t.getSeconds(), 2)}`;
                break;
            case 'month':
                t = `${t.getFullYear()}-${layui.api.markZero(t.getMonth() + 1, 2)}`;
                break;
            case 'time':
                t = `${layui.api.markZero(t.getHours(), 2)}:${layui.api.markZero(t.getMinutes(), 2)}:${layui.api.markZero(t.getSeconds(), 2)}`;
                break;
            default:
                t = '';
                break;
    ...
    // elem jquery选择器对象
    // options.date 时间对象键值
    function getFormField(elem, options){
        const data = elem.serializeArray()
        options = options || {};
        const dateList = options.date||[]
        let field = {};
        data.forEach(
            (item)=> {
                if( item.name ){
                    if( dateList.includes(item.name) ){
                        field[item.name] = new Date(item.value)
                    } else {
    ...
    function getCompareData(arr1,arr2){
        let obj = {};
        for(let i=0;i<arr1.length;i++){
            for(let j=0;j<arr2.length;j++){
                if(arr1[i].branchOffice == arr2[j].branchOffice){
                    obj[j]=i+1;
                }
            }
        }
     ...
     */
    function getThisDateRange(){
        const nowDate = new Date();
        let nowDateRange = ['', '']
        nowDate.setDate(1);
        nowDateRange[0] = nowDate.toLocaleDateString().replace(/\//g, '-');
        const endDate = new Date(nowDate.getTime());
        endDate.setMonth(endDate.getMonth()+1);
        endDate.setDate(0);
        nowDateRange[1] = endDate.toLocaleDateString().replace(/\//g, '-');
        return nowDateRange; 
    }
     ...
     *      cols：参考表格 
     *      filename: 导出的文件名 
     *      [handleDataList]: 导出前数据列表处理，可插入数据行
     *      [beforeFunc]: 列表数据处理，用于合并单元格等操作
     */
    function exportXLSX({ count=999, url, where, cols=[], filename="未命名", handleDataList, beforeFunc}){
        const XLSX = layui.XLSX;
        if (!XLSX) return false;
        let cols_title = cols.map( i => i.title )
        let datalist = null;
        layui.ajax({
            url: url,
            data: {
        ...
        if (!datalist) return layui.renders.fail('导出请求失败');
        if (datalist.length === 0) return layui.renders.fail("无数据可导出");
        let list = [
            cols_title,
            ...(datalist.map(
                (item, index) => [ ...(cols.map( i => {
                    if (i.type === 'numbers') return (index+1);
                    if (i.templet ) {
                        if (typeof i.templet === 'function' ) return i.templet(item);
                        else return i.templet
                    }
                    if (i.field) return (layui.api.exportChange(item[i.field])||'')
                    return ''
                } ) ) ]
            ))
        ]
        if (typeof handleDataList === 'function') {
            list = handleDataList(list)
        }
        const options = {
            dateNF: 'yyyy/mm/dd'
        }
        const wb = XLSX.utils.book_new();
        const ws = XLSX.utils.aoa_to_sheet(list, options);
        ws['!cols'] = cols.map( i => ({ wpx: i.width || 100 }) )
        if (typeof beforeFunc === 'function') {
            beforeFunc(ws)
        }
        XLSX.utils.book_append_sheet(wb, ws, "SheetJS");
        XLSX.writeFileSync(wb, `${filename}.xlsx`)
    }

    function getMonthRange(minDate,maxDate){
        if (minDate && maxDate) {
    ...
    function getTableTagProcessStatus(d){
        const s = typeof d !== 'object' ? d:  d?.approvalState;
        if (!s && s !== 0) return false;
        return `<span class="tag-process-status" data-status="${s}"></span>`
      }

    exports(MOD_NAME, {
        verifyData: verifyData, 
        verifyDataTypes: verifyDataTypes,
        convertTime: convertTime,
        getFormField: getFormField,
        getTimeYMDHM: getTimeYMDHM,
        getChinaJs: getChinaJs,
        getChinaData: getChinaData,
        getCompareData: getCompareData,
        getThisDateRange: getThisDateRange,
        exportXLSX: exportXLSX,
        getMonthRange: getMonthRange,
        getTableTagProcessStatus: getTableTagProcessStatus,
    });
});
