...
layui.define(["jquery","laytpl"], function (exports) {
    ...
    var TPL_ELEM=function(options,floor,parentStatus) {
        return [
                ...
                '<div class="eleTree-node" data-'+options.request.key+'="{{d[i]["'+options.request.key+'"]}}" eletree-floor="'+floor+'" style="display: none;">',
                    '<div class="eleTree-node-content" style="padding-left: '+(options.indent*floor)+'px;">',
                        '<span class="eleTree-node-content-icon">',
                            ...
                            '<i class="icon-extend {{ d[i].none?"":d[i].companyName==="xxx"?"icon-extend-bloc":d[i].companyName?"icon-extend-company": d[i].roleName?"icon-extend-role": d[i].departmentName? "icon-extend-department": ""}}"></i> ',
                        '</span>',
                        function() {
            ...
            '<span class="eleTree-node-content-label" data-id="{{d[i]["' + options.request.key +'"]}}">{{d[i]["'+options.request.name+'"]}}</span>',
                    '</div>',
    ...
    Class.prototype={
        ...
        config: {
            ...
            unCheckAll: false,          //1 在显示复选框的情况下，是否启用单选，默认为 false
        ...
        ajaxGetData: function() {
            ...
            $.ajax({
                ...
                ,success: function(res){
                    ...
                    // 后台接口返回根节点数据对应的name和id为null时直接赋值为空，避免节点出现null节点
        ...
        checkboxEvent: function() {
            ...
            options.elem.on("click",checkOnClickNode,function(e,type) {
                ...
                if(inp.checked){
                    //单选
                    if (options.unCheckAll) _self.unCheckNodes();
                    // 反选自身
