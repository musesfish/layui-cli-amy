...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (e) {
  ...
  var t = layui.$,
    ...
    return i && (c.config[i] = t), {
      ...
      v = 'layui-table-edit',
      m = function m(e) {
    return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ', '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<thead>', '{{# layui.each(d.data.cols, function(i1, item1){ }}', '<tr>', '{{# layui.each(item1, function(i2, item2){ }}', '{{# if(item2.fixed && item2.fixed !== "right"){ left = true; } }}', '{{# if(item2.fixed === "right"){ right = true; } }}', (e = e || {}).fixed && 'right' !== e.fixed ? '{{# if(item2.fixed && item2.fixed !== "right"){ }}' : 'right' === e.fixed ? '{{# if(item2.fixed === "right"){ }}' : '', '{{# var isSort = !(item2.colGroup) && item2.sort; }}', '<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} {{#if(item2.colspan){}} colspan="{{item2.colspan}}"{{#} if(item2.rowspan){}} rowspan="{{item2.rowspan}}"{{#}}} {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}">', '<div class="layui-table-cell laytable-cell-', '{{# if(item2.colGroup){ }}', 'group', '{{# } else { }}', '{{d.index}}-{{i1}}-{{i2}}', '{{# if(item2.type !== "normal"){ }}', ' laytable-cell-{{ item2.type }}', '{{# } }}', '{{# } }}', '" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>', '{{# if(item2.type === "checkbox"){ }}', '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}>', '{{# } else if(item2.type === "colcheckbox"){ }}' + '<div class="col-checkbox">', '<input type="checkbox" name="layTableColCheckbox" title="{{item2.title||\"\"}}" data-field="{{item2.field}}" lay-filter="layTableColAllChoose" lay-skin="primary" {{# if(item2.disabled){ }}disabled{{# } }}  {{# if(item2[d.data.checkName]){ }}checked{{# }; }}>', '</div>', '{{# } else { }}', '<span>{{item2.title||""}}</span>', '{{# if(isSort){ }}', '<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc" title="升序"></i><i class="layui-edge layui-table-sort-desc" title="降序"></i></span>', '{{# } }}', '{{# } }}', '</div>', '</th>', e.fixed ? '{{# }; }}' : '', '{{# }); }}', '</tr>', '{{# }); }}', '</thead>', '</table>'].join('');
  },
  ...
  C.prototype.config = {
    ...
    if (a[0]) {
      var n = t.split('-'),
          o = i.cols[n[0]][n[1]];
      e ? l-- : l++, a.attr('colspan', l), a[l < 1 ? 'addClass' : 'removeClass'](h), o.colspan = o.colspan || l, o.hide = l < 1;
          ...
          if (void 0 !== p && null !== p || (p = ''), !d.colGroup) {
            var v = ['<td data-field="' + c + '" data-key="' + f + '" ' + function () {
              ...
              switch (d.type) {
                ...
                case 'colcheckbox':
                  return ['<div class="col-checkbox">', '<input type="checkbox" name="layTableColCheckbox" data-field="' + d.field + '" lay-skin="primary" title="" ' + (d.disabled || n[d.field] === 1 ? ' disabled ' : '') + (n[d.field] === 1 ? 'checked' : '') + '>', '</div>'].join(" ");
    ...
    return c.key = r.id || r.index, s.cache[c.key] = y, c.layPage[0 == o || 0 === y.length && 1 == n ? 'addClass' : 'removeClass'](h), d ? m() : 0 === y.length ? (c.renderForm(), c.layFixed.remove(), c.layMain.find('tbody').html(''), c.layMain.find('.' + u).remove(), c.layHeader.addClass('overflow-x-auto'), c.layMain.append('<div class="layui-none">' + r.text.none + '</div>')) : (m(), c.renderTotal(y), void (r.page && (r.page = t.extend({
      ...
      jump: function jump(e, t) {
        t || (c.page = e.curr, r.limit = e.limit, c.pullData(e.curr));
      }
    ...
    i[0] && (s.checkStatus(e.key).isAll ? (i[0].checked || (i.prop('checked', !0), e.renderForm('checkbox')), a(!0)) : (i[0].checked && (i.prop('checked', !1), e.layHeader.find("input[name='layTableColCheckbox']").attr("checked", !1), e.renderForm('checkbox')), a(!1)));
    ...
    a.layTool.on('click', '*[lay-event]', function (e) {
      ...
      var i = t(this),
        ...
        n.html(l), i.find('.layui-table-tool-panel')[0] || i.append(n), a.renderForm(), isShowCols = true;
    ...
    a.elem.on('click', 'input[name="layTableCheckbox"]+', function () {
      ...
      o ? (i.each(function (e, t) {
        !t.disabled && (t.checked = n, a.setCheckData(e, n));
      }), !n ? c.each(function (e, t) {
        !t.disabled && (t.checked = n), a.setCheckData(e, n);
      }) : null, a.syncCheckAll(), a.renderForm('checkbox')) : (a.setCheckData(l, n), !n && tr.each(function (e, t) {
        !t.disabled && (t.checked = n);
      }), a.syncCheckAll(), a.renderForm('checkbox'));
      layui.event.call(this, r, 'checkbox(' + g + ')', b.call(this, {
        checked: n,
        type: o ? 'all' : 'one'
      }));
      ...
      o ? (i.each(function (e, d) {
        ...
        a.setCheckData(e, n);
        a.syncCheckAll();
      }), a.renderForm('checkbox')) : (a.setCheckData(l, n, k), n && (p[l].checked = n), n && _f && (p[l + p.length / 2].checked = n), i.each(function (e, d) {
        if (d.dataset.field === k && !d.disabled) {
          if (!d.checked) {
            _sel = false;
            return false;
    ...
    n.href = 'data:' + l + ";charset=utf-8,\uFEFF" + encodeURIComponent(function () {
      ...
      return layui.each(t, function (t, l) {
        ...
        'object' == _typeof(e) ? (layui.each(e, function (e, a) {
          ...
          a.field && 'normal' == a.type && !a.hide && (0 == t && i.push(a.title || ''), typeof l[a.field] === 'string' ? l[a.field] = l[a.field].replace(/,/g, "，") : null, n.push(l[a.field]));
        }), a.push(n.join(','));
      }), i.join(',') + '\r\n' + a.join('\r\n');
