...
layui.define(["jquery"], function (exports) {
  ...
  function Step(option) {
    ...
    this.elem = option.elem; // this.methods=option.methods?option.methods:"";
  ...
  Step.prototype = {
    constructor: Step,
    // 初始化参数数据
    parameterInit: function parameterInit() {
      var self = this;
    ...
    domRender: function domRender() {
      var self = this;
      var titleStr = '<div class="layui-step-title layui-clear">' + '<div class="layui-step-title-item step-first"';
      ...
      titleStr += '<div class="step-icon">' + '<i>1</i>' + '</div>' + '<div class="step-text">' + this.title[0] + '</div>' + '<div class="step-description">' + this.description[0] + '</div>' + '</div>';
      ...
      for (var i = 1; i < this.title.length - 1; i++) {
        ...
        titleStr += '<div class="step-icon">' + '<i>' + (i + 1) + '</i>' + '</div>' + '<div class="step-text">' + this.title[i] + '</div>' + '<div class="step-description">' + this.description[i] + '</div>' + '</div>';
      }

      ;
      titleStr += '<div class="layui-step-title-item step-last" style="width: ' + 100 / this.len + '%;">' + '<div class="step-icon">' + '<i>' + this.len + '</i>' + '</div>' + '<div class="step-text">' + this.title[this.title.length - 1] + '</div>' + '<div class="step-description">' + this.description[this.title.length - 1] + '</div>' + '</div>' + '</div>';
      $(this.elem).prepend(titleStr); // 生成三角
    ...
    changeStep: function changeStep() {
      ...
      this.canIconClick ? function () {
        $(self.elem).on("click", ".layui-step-title-item .step-icon", function () {
          var index = $(this).parent(".layui-step-title-item").index() + 1; // 判断点击的是否为disabled
    ...
    // 是否严格按照步骤条顺序执行步骤
    openStepLevel: function openStepLevel() {
      var self = this;
      this.isOpenStepLevel ? function () {
        ...
        $(self.elem).off().on("click", ".layui-step-title-item .step-icon", function () {
          var index = $(this).parent(".layui-step-title-item").index() + 1; // 判断如果当前点击的步骤超过已走过的最大步，则不跳转
    ...
    prev: function prev() {
      ...
      this.reInit(); // 先保存当前位置，若前面的全都已经禁用，则可以回到当前位置

      var origin = this.currentStep;
      this.PrevGo(origin);
    },
    PrevGo: function PrevGo(origin) {
      this.currentStep--; // 判断前面的是否全都已经禁用
    ...
    next: function next() {
      ...
      this.reInit(); // 先保存当前位置，若后面的全都已经禁用，则可以回到当前位置

      var origin = this.currentStep;
      this.nextGo(origin);
    },
    nextGo: function nextGo(origin) {
      ...
      this.currentStep++; // 判断后面的是否全都已经禁用
  ...
  var step = {
    ...
    disabled: function disabled(i) {
      stepObj.disabled(i);
    },
    abled: function abled(i) {
      stepObj.abled(i);
    }
  };
  exports('step', step);
