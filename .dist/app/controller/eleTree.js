...
layui.define(["jquery", "laytpl"], function (exports) {
  ...
  var thisTree = function thisTree() {
    var _self = this;

    var options = _self.config; // 暴漏外面的方法
    ...
    return {
      ...
      updateKeySelf: function updateKeySelf(key, data) {
        if (options.data.length === 0) return;
      ...
      // 取消选中
      unCheckNodes: function unCheckNodes() {
        if (options.data.length === 0) return;
  ...
  var TPL_ELEM = function TPL_ELEM(options, floor, parentStatus) {
    return ['{{# for(var i=0;i<d.length;i++){ }}', '<div class="eleTree-node" data-' + options.request.key + '="{{d[i]["' + options.request.key + '"]}}" eletree-floor="' + floor + '" style="display: none;">', '<div class="eleTree-node-content" style="padding-left: ' + options.indent * floor + 'px;">', '<span class="eleTree-node-content-icon">', '<i class="layui-icon layui-icon-triangle-r ', function () {
      if (options.lazy) {
        var str = ['{{# if(!d[i]["' + options.request.isLeaf + '"]){ }}', 'lazy-icon" ></i>', '{{# }else{ }}', 'leaf-icon" style="color: transparent;" ></i>', '{{# } }}'].join("");
        return str;
      }

      return ['{{# if(!d[i]["' + options.request.children + '"] || d[i]["' + options.request.children + '"].length===0){ }}', 'leaf-icon" style="color: transparent;"', '{{# } }}', '"></i>'].join("");
    }(), '<i class="icon-extend {{ d[i].none?"":d[i].companyName==="xxx"?"icon-extend-bloc":d[i].companyName?"icon-extend-company": d[i].roleName?"icon-extend-role": d[i].departmentName? "icon-extend-department": ""}}"></i> ', '</span>', function () {
      if (options.showCheckbox) {
        ...
        return ['<input type="checkbox" name="eleTree-node" eleTree-status=' + status + ' class="eleTree-hideen ', '{{# if(d[i]["' + options.request.disabled + '"]) { }}', 'eleTree-disabled', '{{# } }}', '" />'].join("");
      }

      return '';
    }(), '<span class="eleTree-node-content-label" data-id="{{d[i]["' + options.request.key + '"]}}">{{d[i]["' + options.request.name + '"]}}</span>', '</div>', '<div class="eleTree-node-group">', '</div>', '</div>', '{{# } }}'].join("");
  };
  ...
  Class.prototype = {
    ...
    config: {
      ...
      //1 是否在第一次展开某个树节点后才渲染其子节点
      highlightCurrent: false,
      //1 是否高亮当前选中节点，默认值是 false。
      defaultExpandAll: false,
      //1 是否默认展开所有节点
      expandOnClickNode: true,
      //1 是否在点击节点的时候展开或者收缩节点， 默认值为 true，如果为 false，则只有点箭头图标的时候才会展开或者收缩节点。
      checkOnClickNode: false,
      //1 是否在点击节点的时候选中节点，默认值为 false，即只有在点击复选框时才会选中节点。
      defaultExpandedKeys: [],
      //1 默认展开的节点的 key 的数组
      ...
      //1 节点是否可被选择
      checkStrictly: false,
      //1 在显示复选框的情况下，是否严格的遵循父子不互相关联的做法，默认为 false
      defaultCheckedKeys: [],
      //1 默认勾选的节点的 key 的数组
      accordion: false,
      //1 是否每次只打开一个同级树节点展开（手风琴效果）
      indent: 16,
      //1 相邻级节点间的水平缩进，单位为像素
      lazy: false,
      //1 是否懒加载子节点，需与 load 方法结合使用
      load: function load() {},
      //1 加载子树数据的方法，仅当 lazy 属性为true 时生效
      draggable: false,
      //1 是否开启拖拽节点功能
      contextmenuList: [],
      //1 启用右键菜单，支持的操作有："copy","add","edit","remove"
      unCheckAll: false,
      //1 在显示复选框的情况下，是否启用单选，默认为 false
      method: "get",
      url: "",
      contentType: "",
      headers: {},
    ...
    render: function render() {
      ...
      this.filter = options.elem.attr("lay-filter"); // 判断加载方式
    ...
    ajaxGetData: function ajaxGetData() {
      ...
      if (!options.url) {
        options.elem.html("<div style='text-align: center;height: 30px;line-height: 30px;color: #888;'>" + options.emptText + "</div>");
      }
      ...
      $.ajax({
        ...
        success: function success(res) {
          var dataName = options.response.dataName;
          var data; // 取更深层级数据

          if (Array.isArray(dataName)) {
            data = res[dataName[0]];

            for (var i = 1, l = dataName.length; i < l; i++) {
          ...
          if (res[options.response.statusName] != options.response.statusCode || !data) {
            hint.error("请检查数据格式是否符合规范");

            if (data == null) {
              options.elem.html("<div style='text-align: center;height: 30px;line-height: 30px;color: #888;'>无数据</div>");
            }
          ...
          resetOptions[options.request.name] = '请选择';
          resetOptions[options.request.key] = '';
          resetOptions['none'] = 1;
          if (options.data === null) options.data = []; // 后台接口返回根节点数据对应的name和id为null时直接赋值为空，避免节点出现null节点

          if (data[options.request.name] === null && data[options.request.key] === null) data = [];
          options.data = Array.isArray(data) ? data : [data];
          options.data.unshift(resetOptions);
    ...
    // 下拉
    eleTreeEvent: function eleTreeEvent() {
      var _self = this;

      var options = this.config; // 下拉
      ...
      options.elem.on("click", expandOnClickNode, function (e) {
        ...
        var el = eleTreeNodeContent.children(".eleTree-node-content-icon").children(".layui-icon"); // 添加active背景
    ...
    // checkbox选中
    checkboxEvent: function checkboxEvent() {
      var options = this.config;

      var _self = this;

      var checkOnClickNode = options.checkOnClickNode ? ".eleTree-node-content" : ".eleTree-checkbox"; // input添加属性eleTree-status：即input的三种状态，"0":未选中，"1":选中，"2":子孙部分选中

      options.elem.on("click", checkOnClickNode, function (e, type) {
        e.stopPropagation();
        var eleTreeNodeContent = $(this).parent(".eleTree-node").length === 0 ? $(this).parent(".eleTree-node-content") : $(this);
        var checkbox = eleTreeNodeContent.children(".eleTree-checkbox");
        if (checkbox.hasClass("eleTree-checkbox-disabled")) return; // 获取点击所在数据

        var node = eleTreeNodeContent.parent(".eleTree-node"); // var d=_self.reInitData(node).currentData;
        // 实际的input

        var inp = checkbox.siblings(".eleTree-hideen").get(0);
        var childNode = eleTreeNodeContent.siblings(".eleTree-node-group").find("input[name='eleTree-node']"); // 添加active背景
        ...
        if (inp.checked) {
          //单选
          if (options.unCheckAll) _self.unCheckNodes(); // 反选自身

          $(inp).removeAttr("checked").attr("eleTree-status", "0"); // 点击祖父层选中子孙层
          ...
          $(inp).prop("checked", "checked").attr("eleTree-status", "1"); // 点击祖父层选中子孙层
        ...
        var eleNode = eleTreeNodeContent.parent(".eleTree-node"); // 点击子孙层选中祖父层(递归)

        if (!options.checkStrictly) {
          var siblingNode = eleNode.siblings(".eleTree-node"); // 点击子孙层选中祖父层(递归)
    ...
    // 通过子元素选中祖父元素
    selectParents: function selectParents(inp, eleNode, siblingNode) {
      // inp: 实际input(dom元素)
      ...
      while (Number(eleNode.attr("eletree-floor")) !== 0) {
        ...
        var parentInput = eleNode.parent(".eleTree-node-group").siblings(".eleTree-node-content").children("input[name='eleTree-node']"); // 父元素的checkbox替代

        var parentCheckbox = parentInput.siblings(".eleTree-checkbox"); // 子都选中则选中父
    ...
    // 初始展开所有
    expandAll: function expandAll(data, arr, floor, isMethodsExpandAll) {
      var options = this.config;
          ...
          laytpl(TPL_ELEM(options, floor)).render(val[options.request.children], function (string) {
            ...
            if (options.defaultExpandAll || isMethodsExpandAll) {
              ...
              if ($.inArray(id, options.defaultExpandedKeys) !== -1) {
                el.siblings(".eleTree-node-content").children(".eleTree-node-content-icon").children(".layui-icon").addClass("icon-rotate");
                el.children().show(); // 展开子项是否继续展开祖父项
    ...
    defaultChecked: function defaultChecked() {
      var options = this.config;
      if (options.defaultCheckedKeys) if (options.defaultCheckedKeys.length === 0) {
        return false;
      } // 判断是否父子无关
    ...
    // 自定义checkbox解析
    checkboxRender: function checkboxRender() {
      var options = this.config;
    ...
    // 通过dom节点找对应数据
    reInitData: function reInitData(node) {
      var options = this.config;
      var i = node.index();
      ...
      arr = arr.reverse();
      var oData = this.config.data; // 当前节点的父节点数据

      var parentData = oData[arr[0]]; // 当前节点的data数据
    ...
    // 通过key查找数据
    keySearchToOpera: function keySearchToOpera(key, callback) {
      var options = this.config;
    ...
    updateKeyChildren: function updateKeyChildren(key, data) {
      ...
      this.keySearchToOpera(key, function (d, obj) {
        // 数据更新
        d[obj.i][options.request.children] = data; // dom更新
    ...
    updateKeySelf: function updateKeySelf(key, data) {
      ...
      data[options.request.disabled] && node.children(".eleTree-hideen").addClass("eleTree-disabled").siblings(".eleTree-checkbox").addClass("eleTree-checkbox-disabled"); // 数据更新
    ...
    remove: function remove(key) {
      ...
      var pElem = node.parent(".eleTree-node-group"); // 数据删除
    ...
    append: function append(key, data) {
      var options = this.config;
      var node = $("[data-" + options.request.key + "='" + key + "']");
      var floor = Number(node.attr("eletree-floor")) + 1; // 数据更新
      ...
      this.keySearchToOpera(key, function (d, obj) {
        ...
        var arr = d[obj.i][options.request.children]; // 添加之后长度为1，则原来没有三角，添加三角
    ...
    insertBefore: function insertBefore(key, data) {
      var options = this.config;
      var node = $("[data-" + options.request.key + "='" + key + "']");
      var floor = Number(node.attr("eletree-floor")); // 数据更新
    ...
    insertAfter: function insertAfter(key, data) {
      ...
      this.unCheckNodes();
      this.defaultChecked(); // if(!options.lazy){
    ...
    getChecked: function getChecked(leafOnly, includeHalfChecked) {
      ...
      if (leafOnly) {
        el = $(".layui-icon.leaf-icon").parent(".eleTree-node-content-icon").siblings("input.eleTree-hideen[eletree-status='1']");
    ...
    setChecked: function setChecked(arr) {
      var options = this.config;
      this.unCheckNodes();
      if (arr.length == 0) options.defaultCheckedKeys = []; //全不选
      else {
    ...
    unExpandAll: function unExpandAll() {
      var options = this.config;
      options.elem.find(".layui-icon.icon-rotate").removeClass("icon-rotate").parent(".eleTree-node-content-icon").parent(".eleTree-node-content").siblings(".eleTree-node-group").children(".eleTree-node").hide();
    },
    nodeEvent: function nodeEvent() {
      var _self = this;

      var options = this.config; // 节点被点击的回调事件
      ...
      options.draggable && options.elem.on("mousedown", ".eleTree-node-content", function (e) {
        ...
        $(document).on("mousemove", function (e) {
          ...
          cloneNode.remove();
          options.elem.css("user-select", "auto"); // 当前点击的是否时最外层

          var isCurrentOuterMost = eleNode.parent().get(0).isEqualNode(options.elem.get(0)); // 目标是否时最外层
          ...
          var tarData = targetData.currentData; // 当前是否是最外层

          isCurrentOuterMost ? options.data.splice(currIndex, 1) : currList.splice(currIndex, 1); // 目标是否是最外层
          ...
          temNode.children(".eleTree-node-content").css("padding-left", floor * options.indent + "px"); // 通过floor差值计算子元素的floor
          ...
          leaf && groupNode.siblings(".eleTree-node-content").children(".eleTree-node-content-icon").children(".layui-icon").removeClass("icon-rotate").css("color", "transparent"); // 当前的增加三角

          var cLeaf = target.children(".eleTree-node-group").children(".eleTree-node").length === 0;
          !cLeaf && target.children(".eleTree-node-content").children(".eleTree-node-content-icon").children(".layui-icon").addClass("icon-rotate").removeAttr("style");
    ...
    rightClickMenu: function rightClickMenu() {
      var _self = this;
      ...
      options.elem.off("contextmenu").on("contextmenu", ".eleTree-node-content", function (e) {
        ...
        $("#tree-menu li.edit").off().on("click", function (e) {
          ...
          label.siblings(".eleTree-node-content-input").focus().select().off().on("blur", function () {
            ...
            if (isStop) return; // 修改数据

            _self.reInitData(eleNode).currentData[options.request.name] = val; // 修改dom
