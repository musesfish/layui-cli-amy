      ...
      dom.find('.select-table-section').each(function () {
        ...
        domOne.each(function () {
          if ($(this).hasClass('layui-form-checked')) {
            ...
            item.value = $.trim($(this).text()); //检测子栏目
    ...
    unite: function unite() {
      ...
      for (var i = 0; i < this.dataArr.length; i++) {
        ...
        if (this.dataArr[i].id == 10 || this.dataArr[i].id == 11 || this.dataArr[i].id == 30 || this.dataArr[i].id == 60) {
          direct_controlled += '<div class="layui-unselect layui-form-checkbox select-one" data-id="' + this.dataArr[i].id + '" lay-skin="primary">' + '      <span><b>' + this.dataArr[i].value + '</b></span><i class="layui-icon layui-icon-ok"></i>' + '</div>';
        } else {
          province += '<div class="layui-row select-table-section">' + '                    <div class="select-table-label layui-col-md2 ">' + '                        <div class="layui-unselect layui-form-checkbox select-one" data-id="' + this.dataArr[i].id + '" lay-skin="primary">' + '                            <span><b>' + this.dataArr[i].value + '</b></span><i class="layui-icon layui-icon-ok"></i>' + '                        </div>' + '                    </div>' + '                    <div class="select-table-content layui-col-md10">' + function () {
            if (this.dataArr[i].childs && this.dataArr[i].childs.length > 0) {
              var temp = '';

              for (var j = 0; j < this.dataArr[i].childs.length; j++) {
                temp += '                        <div class="layui-unselect layui-form-checkbox select-two" data-id="' + this.dataArr[i].childs[j].id + '" lay-skin="primary">' + '                            <span>' + this.dataArr[i].childs[j].value + '</span><i class="layui-icon layui-icon-ok"></i>' + '                        </div>';
              }
      ...
      html += '<div class="layui-row select-table-section">' + '<div class="select-table-label layui-col-md12 ">' + direct_controlled + '</div>' + '</div>' + province;
      var dom = $(this.elem); //渲染页面
      ...
      if (this.setValues && this.setValues.length > 0) {
        dom.find('.select-table-section').each(function () {
          ...
          var $two = $(this).find('.select-two'); // for (var i = 0; i < that.setValues.length; i++) {
