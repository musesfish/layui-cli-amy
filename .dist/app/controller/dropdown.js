...
layui.define(["jquery"], function (exports) {
  var $ = layui.jquery; // 记录dropdown的上一次点击的按钮dom

  var dropdownTarget = "";

  var dropdown = function dropdown(elem, data, callback) {
    var $elem = null;
    ...
    $elem.off("click").on("click", function (event) {
      ...
      str += '</ul>';
      $(document.body).append(str); // 位置与事件绑定
  ...
  var documentEvent = function documentEvent() {
    $(document.body).on("click", '.urp-dropdown:not(.urp-dropdown-table)', function (event) {
      ...
      var onOff = true; // 判断当前是否打开，是的话，直接关闭，否则先关闭所有的，再打开当前的
