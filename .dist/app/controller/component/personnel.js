...
layui.extend({
  ...
  var url = '/structure/getOrganizationalsByRoleIdAndCustomerId',
      // 人员选择
  query = {
    'roleId': urlParmas.r
  };
  if (urlParmas.c) query.customerId = urlParmas.c; // 根据分部id获取人员
    ...
    if (_typeof(data) === 'object' && data != null) {
      if (!Array.isArray(data)) {
        if (data.hasOwnProperty('companyName')) {
          ...
          d['departmentName'] = data['departmentName']; // 根级分公司
  ...
  $('#select1').on("dblclick", "div", function (e) {
    // 绑定双击事件
    clearTimeout(clickFlag);
  ...
  function conversionToTemp(d, search) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      temp.push("<div data-value='".concat(d[i].id || '', "' data-label='").concat(d[i].userName || '', "' data-company=\"").concat(companyName, "\" data-department=\"").concat(departmentName, "\">").concat(d[i].userName, "<br>\n                <span class=\"sel-fc-sub\">").concat(departmentName, "&nbsp;&nbsp;").concat(companyName, "</span>\n            </div>"));
    }
