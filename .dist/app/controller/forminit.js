...
layui.define(['jquery', 'form', 'admin', 'layedit', 'form', 'enhanceform', 'fwb'], function (exports) {
  var $ = layui.jquery,
      ...
      //表单增强插件，给元素赋初始值，因此版本为2.2.6，所以需要用此插件进行给表单赋初始值，从2.3.0开始可直接用form.val('lay-filter的值', object)对表单元素赋初始值;参考网址：http://fly.layui.com/jie/23002/
  layer = layui.layer;
  form = layui.form;
  var forminit = {
    elem: [],
    ...
    inputtest: function inputtest(data) {
      //判断输入的格式
    ...
    setCheckB1: function setCheckB1() {
      //渲染文checkbox数据1
      var that = this;
      var initarr = [];
    ...
    setCheckB2: function setCheckB2() {
      //渲染文checkbox数据2
      var that = this;
    ...
    setFwb: function setFwb() {
      ...
      var editindex = layedit.build(id); //重置富文本编辑器的信息
    ...
    getFwbname: function getFwbname() {
      //返回富文本编辑器的name名
      return this.fwbname;
    },
    setLimit: function setLimit() {
      ...
      for (var i = 0; i < that.limitarea.length; i++) {
        ...
        box.find('.inputword').css({
          "color": "#CC0000"
        });
        var wordCount = box,
            textArea = wordCount.find("textarea"),
            word = wordCount.find(".inputword"); //调用
