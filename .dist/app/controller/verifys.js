...
layui.define(['form'], function (exports) {
  ...
  var reg = {
    // 银行账户
    appBankAccount: function appBankAccount(val) {
      if (!/^[0-9]{12,20}$/.test(val)) {
        return '账号格式为12-20位数字';
      }
    },
    // 银行账户可能更多位数
    appBankAccountMore: function appBankAccountMore(val) {
      if (!/^[0-9]{1,30}$/.test(val)) {
        return '账号格式为1-30位数字';
      }
    },
    // 开户银行
    appBankName: function appBankName(val) {
      if (!/^[\u4e00-\u9fa5]{4,25}$/.test(val)) {
        return '请输入4-25位的汉字';
      }
    },
    // 1到15个汉字
    onetoOneFive: function onetoOneFive(val) {
    ...
    // 验证身份证返回出生日期
    idCrads: function idCrads(value) {
     ...
     * 最大数值为 万亿（16位数字）
    */
    converAmount: function converAmount(num) {
    ...
    //验证汉字字数
    DataLength: function DataLength(fData) {
      var intLength = 0;
      if (!fData) return;

      for (var i = 0; i < fData.length; i++) {
        if (fData.charCodeAt(i) < 0 || fData.charCodeAt(i) > 255) // intLength=intLength+2 
          intLength = intLength + 1;else intLength = intLength + 1;
      }
     ...
     * 浮点数据
    */
    converFloat: function converFloat(s) {
    ...
    */
    testInteger: function testInteger(num, eq, min, max) {
      if (min && max) {
     ...
     * >=999 <=1 的整数
    */
    test1to999: function test1to999(num) {
    ...
    */
     ...
     * 不连着一样的数字
    */
    notTheSame: function notTheSame(num) {
      if (/^(\d)\1+$/.test(num)) {
        return 1;
      } else {
        return 0;
     ...
     * 正负正数，正负小数
    */
    isAllNum: function isAllNum(val) {
    ...
    */
    ...
    isNums: function isNums(val, node) {
      ...
      if (!isOK) return "\u8BF7\u8F93\u5165 ".concat(min, " \u5230 ").concat(max, " \u4E4B\u95F4\u7684\u503C");
    },
    isCN: function isCN(val, node) {
      ...
      if (!isOK) return "\u8BF7\u8F93\u5165 ".concat(min, " \u5230 ").concat(max, " \u4E4B\u95F4\u7684\u4E2D\u6587");
    },
    receivables: function receivables(val) {
      val = !val ? '' : val;
    ...
    businessVeri: function businessVeri(val) {
      if (!val) return "业务类型不能为空";
    },
    // 口号
    slogan: function slogan(val) {
      if ((val === null || val === void 0 ? void 0 : val.length) > 50) return '口号长度不能超过50个字';
    },
    // 时间：由数字 、/、. 、-
    ...
    // 最大长度
    strlen: function strlen(val, el) {
      var max = el.dataset.max;
      var msg = el.dataset.msg;
      val = val.trim();
      if (val.length > max) return msg || "\u957F\u5EA6\u4E0D\u80FD\u8D85\u8FC7".concat(max, "\u4E2A\u5B57\uFF01");
    },
    // 联系方式固定电话+手机号码
    contacts: function contacts(val) {
      if (!/(^1\d{10}$)|(^0[\d]{2,3}(-?)[\d]{7,8})$/.test(val)) return '请输入正确的联系方式';
    },
    hideRequired: function hideRequired(val, inp) {
      var value = layui.$(inp).next('input').val();
