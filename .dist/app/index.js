...
layui.extend({
  ...
  document.addEventListener('visibilitychange', function () {
    ...
    if (visible === 'visible') {
      var _layui$store$get;

      var userId = (_layui$store$get = layui.store.get(layui.setter.tableName)) === null || _layui$store$get === void 0 ? void 0 : _layui$store$get.userId;
  ...
  $(document).on("blur", ".layui-input", function () {
    ...
    setTimeout(function () {
      $(_this).parent().find('.shanchu').remove();
    }, 200);
  }); // 点击删除键清除文本框
  ...
  $(document).on('click', ".j-set-apis", function () {
    layer.prompt({
      ...
      if (val) {
        ...
        layer.msg("api: ".concat(val), function () {
          location.reload();
          layer.close(index);
        });
      }
